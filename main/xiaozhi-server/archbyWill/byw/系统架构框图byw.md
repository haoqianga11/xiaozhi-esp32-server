...
│  │  │   端口: 8000    │  │   端口: 8003    │  │  Manage API交互   │            ││
│  │  │  ESP32通信      │  │  OTA下发/视觉API│  │  WebSocket热更新   │            ││
│  │  └─────────────────┘  └─────────────────┘  └─────────────────┘            ││
│  └─────────────────────────────────────────────────────────────────────────────┘│
└─────────────────────────────────────────┬───────────────────────────────────────┘
                                          │ WebSocket 实时通信 / MQTT网关桥接
                                          ▼
┌─────────────────────────────────────────────────────────────────────────────────┐
│                              ESP32 智能硬件设备                                   │
│  ┌─────────────┐    ┌─────────────┐    ┌─────────────┐    ┌─────────────┐     │
...
HTTP 服务目前提供 OTA 下发接口 (`/xiaozhi/ota/`) 与视觉分析接口 (`/mcp/vision/explain`)，当 `server.mqtt_gateway` 配置存在时，会在 OTA 响应中携带 MQTT 连接参数，支持网关在设备与 WebSocket 服务之间桥接音频。
...
│  │                            动态配置更新机制                                  ││
│  │                                                                             ││
│  │  管理员操作 ──► Web界面修改 ──► API保存 ──► WebSocket通知 ──► 热更新执行        ││
│  │                                                                             ││
│  │  ┌─────────────────────────────────────────────────────────────────────────┐││
│  │  │                       配置更新完整链路                                  │││
│  │  │                                                                         │││
│  │  │  ┌─────────────┐    ┌─────────────┐    ┌─────────────┐    ┌─────────────┐ │││
│  │  │  │ 管理员      │    │ manager-web │    │ manager-api │    │ WebSocket推送│ │││
│  │  │  │ 修改AI配置  │──► │ Vue前端界面 │──► │ 后端API     │──► │ update_config│ │││
│  │  │  │ 选择模型    │    │ 表单验证    │    │ 数据验证    │    │ 签名校验     │ │││
│  │  │  │ 更新密钥    │    │ 实时预览    │    │ MySQL存储   │    │ 目标连接     │ │││
│  │  │  └─────────────┘    └─────────────┘    └─────────────┘    └─────────────┘ │││
│  │  │                                                         │                 │││
│  │  │                                                         ▼                 │││
│  │  │  ┌─────────────┐    ┌─────────────┐    ┌─────────────┐                   │││
...
上述流程对应 `core/handle/textHandler/serverMessageHandler.py` 中的 `update_config` 处理：manager-api 通过 WebSocket 向目标实例推送指令，服务端在 `WebSocketServer.update_config()` 内调用 `config/manage_api_client.py` 的 `get_config_from_api()` 拉取最新配置，并在 `asyncio.Lock` 保护下按需重建 VAD、ASR、LLM、Intent、Memory 等模块，实现不中断的热更新。
...
- **实时通信**: WebSocket协议保证低延迟的音频交互，可选MQTT网关桥接
- **动态配置**: WebSocket通知 + manager-api拉取实现运行时更新，无需重启服务
- **多模态支持**: 音频、文本、视觉的统一处理框架

### 🔧 扩展能力
- **新AI服务**: 继承Base类即可接入新的AI提供商
- **功能插件**: 支持自定义插件开发和热加载
- **协议扩展**: 支持IoT、MCP、MQTT网关等多种设备控制协议
- **多端管理**: Web、移动端的统一管理界面
